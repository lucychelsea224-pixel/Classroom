...
// Which Gemini model to use. Resolved fresh on every request, in this
// order, so it can be changed without a redeploy:
//   1. app_settings row 'gemini_model' in the database (editable from
//      the Admin panel → AI Settings — this is the one to change day to day)
//   2. GEMINI_MODEL secret (`supabase secrets set GEMINI_MODEL=...`), for
//      when you'd rather manage it outside the database
//   3. hardcoded fallback below, only used if both of the above are empty
// Check https://aistudio.google.com/ (your own account, logged in) for
// the current free-tier model list — that's the only fully reliable
// source since it changes based on your account, not just the date.
const FALLBACK_GEMINI_MODEL = Deno.env.get("GEMINI_MODEL") || "gemini-2.5-flash-lite";

async function resolveGeminiModel(supabaseUser) {
  try {
    const { data, error } = await supabaseUser
      ...
      .eq("key", "gemini_model")
      .single();
    if (error || !data?.value) return FALLBACK_GEMINI_MODEL;
    return data.value;
  } catch (_err) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { question, subjectContext, history, previewModel } = await req.json();
    if (!question || !question.trim()) {
      return jsonResponse({ error: "No question provided." });
    }

    // Admin-only "test this model before saving it" path — used by the
    // AI Settings tab's Test button. Skips the points system entirely
    // (this isn't a student session spending their daily quota) and
    // uses whatever model string was typed, NOT the saved app_settings
    // value, so the admin can try a model out before committing to it.
    if (previewModel) {
      const { data: profile } = await supabaseUser.from("profiles").select("is_admin").eq("id", user.id).single();
      if (!profile?.is_admin) return jsonResponse({ error: "Only the admin can preview a model." });

      const answer = await callGemini(previewModel, question, subjectContext, history);
      if (answer.error) return jsonResponse({ error: answer.error });
      return jsonResponse({ answer: answer.text, previewedModel: previewModel });
    }
    ...
    const GEMINI_MODEL = await resolveGeminiModel(supabaseUser);
    const result = await callGemini(GEMINI_MODEL, question, subjectContext, history);
    ...
    return jsonResponse({ answer: result.text, remaining });
  } catch (err) {
    return jsonResponse({ error: "Unexpected error: " + err.message });
  }
});

// Shared Gemini call used by both the normal chat path and the admin
// model-preview path above — same request shape, same error handling.
async function callGemini(model, question, subjectContext, history) {
  ...
  try {
    ...
    return { error: "Couldn't reach the AI service: " + fetchErr.message };
  }

  const data = await res.json();
  if (!res.ok) {
    return { error: "AI service error: " + (data?.error?.message || `HTTP ${res.status}`) };
  }

  const answer = data?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!answer) {
    const blockReason = data?.candidates?.[0]?.finishReason;
    return { error: blockReason ? `The AI couldn't answer that (${blockReason}). Try rephrasing.` : "Sorry, I couldn't come up with an answer for that — try rephrasing your question." };
  }
