...
const CACHE_NAME = "classroom-shell-v24";
...
// ---- PUSH ----
// Fires when the send-reminders Edge Function (or any other future
// push source) sends a notification to this device's subscription.
// The payload is a small JSON blob: { title, body, url }.
self.addEventListener("push", (event) => {
  let payload = { title: "Classroom", body: "Come back and keep your streak going!", url: BASE_PATH + "classroom-dashboard.html" };
  try {
...
// at the URL the push payload specified (defaults to the dashboard).
self.addEventListener("notificationclick", (event) => {
  event.notification.close();
  const targetUrl = event.notification.data?.url || BASE_PATH + "classroom-dashboard.html";
